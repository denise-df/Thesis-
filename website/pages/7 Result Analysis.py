...
.mini-card { background:rgba(255,255,255,0.04); border:1px solid rgba(82,183,136,0.15); border-radius:10px; padding:1.3rem; height:100%; }
...
st.markdown(
    ...
    "50,000 deliveries a day, a 100% diesel fleet emitting ~135 tonnes of CO₂ daily. "
    "All figures below describe <b>this</b> operator — they scale with the size of the business.</div>",
    unsafe_allow_html=True
)

st.markdown("<div class='sec-h'>1 · The cost of doing nothing</div>", unsafe_allow_html=True)
st.markdown(
    "<div class='sec-explain'>If the operator keeps its diesel fleet, CO₂ becomes a real bill once EU carbon rules "
    "apply (ETS2, from 2027). That bill grows every year as the price of carbon rises — money lost for doing nothing.</div>",
    unsafe_allow_html=True
)
st.markdown("""
<div class='line-wrap'>
...
st.markdown("<div class='sec-h'>2 · The value of acting</div>", unsafe_allow_html=True)
st.markdown(
    "<div class='sec-explain'>Acting saves money every year. The cheapest move needs no new vehicles at all — just renegotiating "
    "delivery deadlines. The strongest move is going fully electric. Everything in between sits on this line.</div>",
    unsafe_allow_html=True
)
st.markdown("""
<div class='line-wrap'>
...
with chart_col:
    ...
    fig.add_trace(go.Scatter(
        ...
        line=dict(color="#E65100", width=3, dash="dot"),
...
with m2:
    st.markdown("""<div class='mini-card'><div class='mini-lbl'>Full electrification cost</div><div class='mini-num'>€22M</div></div>""", unsafe_allow_html=True)
with m3:
