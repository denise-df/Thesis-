...
# ── Load dynamic KPIs produced by NB06 (fleet_kpis.json) — usati dal tab What-If ──
@st.cache_data
...
/* ── Saving box (REINTEGRATO, usato da What-If e Cost) ── */
...
/* ── Justification boxes (REINTEGRATO, usato da What-If e Cost) ── */
...
# VEHICLE_COSTS (REINTEGRATO): tariffe operative €/km usate solo dal tab
# "Cost Analysis". Indipendenti dai fattori fisici (scale) usati per il CO₂ —
# stessa chiave-veicolo di VEHICLES, cifre puramente economiche (non toccano
...
# ══════════════════════════════════════════════════════════════════════
# TABS (REINTEGRATO): Trip Simulator (contenuto invariato) · What-If ·
# Cost Analysis. Le due sezioni mancanti sono state ricostruite a partire
# dalla versione precedente del file e adattate alla firma attuale di
# calc_co2() (che ora restituisce una tupla (kg, source) invece del solo
# valore kg).
# ══════════════════════════════════════════════════════════════════════
tab_sim, tab_wi, tab_cost = st.tabs(["🚗 Trip Simulator", "🌍 What-If: Fleet Transition", "💰 Cost Analysis"])

# ══════════════════════════════════════════════════════════════════════
# TAB 1 — TRIP SIMULATOR (contenuto invariato rispetto al file caricato)
# ══════════════════════════════════════════════════════════════════════
with tab_sim:
...
# ══════════════════════════════════════════════════════════════════════
# TAB 2 — WHAT-IF: FLEET TRANSITION (REINTEGRATO)
# Logica invariata rispetto alla versione precedente: nessuna chiamata a
# calc_co2() qui, si basa solo su kpi_data / baseline aggregate.
# ══════════════════════════════════════════════════════════════════════
with tab_wi:
...
# ══════════════════════════════════════════════════════════════════════
# TAB 3 — COST ANALYSIS (REINTEGRATO)
# Adattato alla nuova firma calc_co2() → (kg, source): qui interessa solo
# il valore kg, la fonte (ml/fallback) viene scartata con "_".
# Etichetta SLA "Next-Day" rinominata in "Two-Day" per coerenza con il
# resto della pagina e con la tesi (Standard/Two-Day/Express/Same-Day).
# ══════════════════════════════════════════════════════════════════════
with tab_cost:
    ...
    sla_mult_map = {"Standard": 1.0, "Two-Day": 1.05, "Express": 1.20, "Same-Day": 1.40}
    sla_mult = sla_mult_map[cost_sla_sel]
    total_ops_cost = operational_cost * sla_mult

    co2_kg_cost, _ = calc_co2(cost_veh, "Moderate", cost_sla_sel, cost_dist)
    ...
    co2_optimal, _  = calc_co2(best_veh_cost, "Moderate", "Standard", cost_dist)
    ...
    for vname, vrate in VEHICLE_COSTS.items():
        co2_v, _ = calc_co2(vname, "Moderate", cost_sla_sel, cost_dist)
