...
# =====================================================================
# 1. ENVIRONMENT SETUP & GRAPH DOWNLOAD
# =====================================================================
print("Initializing Urban Environment (Rome, EUR)...")
center_point = (41.8368, 12.4663) 

print("Downloading street network from OpenStreetMap (drive)...")
# Using a 3.5km radius for dense urban environment
G = ox.graph_from_point(center_point, dist=3500, network_type='drive')

# Central Hub (Real coordinates: Poste Italiane CPD Eur)
hub_coords = (41.8565, 12.4738) 
hub_node = ox.distance.nearest_nodes(G, hub_coords[1], hub_coords[0])

print("✅ Urban Graph loaded successfully.")
...
print(f"📦 Generated {len(daily_orders)} random daily orders.")
...
print("="*45)
print("FINAL RESULTS: ROUTING TOPOLOGY IMPACT")
print("="*45)
...
title_html_pro = f'''
     ...
     <span style="font-size:13px; color:#52B788;">Urban Road Network (Rome, EUR)</span>
     ...
     <div style="margin-top: 15px; padding: 10px; background-color: #f8f9fa; border-left: 4px solid #e6194b; border-radius: 4px;">
         <span style="font-size:12px; color:#666;">EMISSION GAP:</span><br>
         <span style="font-size:22px; color:#d32f2f; font-weight: 800;">+{diff_perc:.1f}% CO₂</span>
     </div>
     </div>
     '''
m_urban.get_root().html.add_child(folium.Element(title_html_pro))

m_urban.save("thesis_result_map_REAL_ROADS_urban.html")
print("✅ Map successfully generated as 'thesis_result_map_REAL_ROADS_urban.html'")
